.PHONY: help install install-dev install-modal run-all \
        ...
        lint format test check setup start stop status
...
help:
	...
	@echo ""
	@echo "CLI (após install):"
	@echo "  make setup          - Wizard interativo (equivalente: rlm setup)"
	@echo "  make start          - Inicia servidor RLM (equivalente: rlm start)"
	@echo "  make stop           - Para o servidor    (equivalente: rlm stop)"
	@echo "  make status         - Status dos processos (equivalente: rlm status)"
	@echo ""
	@echo "Examples:"
