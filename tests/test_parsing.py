...
multiline answer)"""
    ...
    def test_final_var_retrieves_actual_variables_from_environment(self):
        ...
        try:
            ...
            # Test that non-existent variable returns None (loop continua, não encerra com erro)
...
class TestFindFinalAnswerRobustness:
    """Testes de robustez para o fix do FINAL_VAR (variável ausente não encerra loop)."""

    def test_final_var_missing_variable_returns_none(self):
        """FINAL_VAR com variável inexistente deve retornar None, não string de erro."""
        from unittest.mock import Mock

        mock_env = Mock()
        mock_env.execute_code.return_value = REPLResult(
            stdout="Error: Variable 'missing' not found. Available variables: [].",
            stderr="",
            locals={},
        )
    ...
    def test_final_var_empty_stdout_returns_none(self):
        """FINAL_VAR com stdout vazio (exceção no execute_code) deve retornar None."""
        from unittest.mock import Mock
        ...
        result = find_final_answer("FINAL_VAR(broken)", environment=mock_env)
        # Antes do fix, retornava o stderr como resposta final — agora deve ser None
        assert result is None, f"Expected None, got {result!r}"

    def test_final_var_valid_variable_still_works(self):
        """Regressão: FINAL_VAR com variável válida ainda retorna o valor correto."""
        from unittest.mock import Mock
        ...
        result = find_final_answer("FINAL_VAR(answer)", environment=mock_env)
        assert result == "the real answer", f"Expected 'the real answer', got {result!r}"

    def test_final_var_error_prefix_returns_none(self):
        """Qualquer stdout que comece com 'Error:' deve retornar None."""
        from unittest.mock import Mock

        mock_env = Mock()
        mock_env.execute_code.return_value = REPLResult(
            stdout="Error: Variable 'x' not found. Available variables: ['y', 'z'].",
            stderr="",
            locals={},
        )
        result = find_final_answer("FINAL_VAR(x)", environment=mock_env)
        assert result is None

    def test_final_var_real_env_missing_var_returns_none(self):
        """Integração com LocalREPL real: variável ausente → None, não string de erro."""
        from rlm.environments.local_repl import LocalREPL

        env = LocalREPL()
        try:
            result = find_final_answer("FINAL_VAR(nao_existe)", environment=env)
            assert result is None, f"Expected None, got {result!r}"
        finally:
            env.cleanup()

    def test_final_var_real_env_defined_var_returns_value(self):
        """Integração com LocalREPL real: variável definida → valor correto."""
        from rlm.environments.local_repl import LocalREPL

        env = LocalREPL()
        try:
            env.execute_code("resposta_final = 'sucesso'")
            result = find_final_answer("FINAL_VAR(resposta_final)", environment=env)
            assert result == "sucesso", f"Expected 'sucesso', got {result!r}"
        finally:
            env.cleanup()
