...
try:
    ...
    _ark_mem = None  # type: ignore[assignment]
...
def cosine_similarity(v1: List[float], v2: List[float]) -> float:
    if not v1 or not v2 or len(v1) != len(v2):
        return 0.0
    if _RUST_AVAILABLE:
        return _ark_mem.cosine_similarity(v1, v2)
        return 0.0
...
class MultiVectorMemory:
    ...
    def __init__(self, db_path: str = "rlm_memory_v2.db", embedding_model: str = "text-embedding-3-small"):
        ...
        # Rust-accelerated HNSW index (lazy-loaded on first search)
        self._vector_index = None  # type: ignore[assignment]
    ...
    def add_memory(
        ...
        # Index into Rust HNSW for fast ANN search
        if _RUST_AVAILABLE and embedding:
            try:
                self._ensure_vector_index(len(embedding))
                self._vector_index.add(memory_id, embedding)  # type: ignore[union-attr]
            except Exception as e:
                mem_log.warn(f"Rust vector index insert failed for {memory_id}: {e}")
    ...
    def deprecate(self, memory_id: str) -> None:
        ...
        with closing(sqlite3.connect(self.db_path)) as conn:
            ...
            conn.commit()
        # Also remove from Rust index
        if _RUST_AVAILABLE and self._vector_index is not None:
            try:
    ...
    def _ensure_vector_index(self, dim: int) -> None:
        """Lazily create and warm-up the Rust HNSW index from SQLite data."""
        if self._vector_index is not None:
            return
        if not _RUST_AVAILABLE:
            return
        self._vector_index = _ark_mem.ArkheVectorIndex(dim=dim)  # type: ignore[union-attr]
        # Bulk-load existing embeddings from SQLite
        try:
            with closing(sqlite3.connect(self.db_path)) as conn:
                conn.row_factory = sqlite3.Row
                rows = conn.execute(
                    "SELECT id, embedding FROM memory_chunks WHERE is_deprecated = 0 AND embedding IS NOT NULL AND embedding != '[]'"
                ).fetchall()
            ids = []
            embeddings = []
            for row in rows:
            ...
            if ids:
                self._vector_index.bulk_add(ids, embeddings)
                mem_log.debug(f"Rust HNSW index warm-loaded with {len(ids)} vectors (dim={dim})")
        except Exception as e:
            mem_log.warn(f"Failed to warm-load Rust vector index: {e}")
    ...
    def search_hybrid(
        ...
        with closing(sqlite3.connect(self.db_path)) as conn:
            ...
            # 2. Vector Search — Rust HNSW (O(log N)) or Python fallback (O(N))
            vec_ranks = {}
            chunk_data = {}

            if _RUST_AVAILABLE and query_embedding:
                # Fast path: Rust ANN index
                try:
                    self._ensure_vector_index(len(query_embedding))
                except Exception:
                    pass

                if self._vector_index is not None:
                    ann_results = self._vector_index.search(query_embedding, top_k=20)
                    # ann_results: List[(id, cosine_distance)]
                    # Load metadata from SQLite only for matched ids
                    matched_ids = [r[0] for r in ann_results]
                    if matched_ids:
                        placeholders = ",".join("?" * len(matched_ids))
                        sql_meta = (
                            f"SELECT id, content, metadata, embedding, session_id, timestamp, importance_score "
                            f"FROM memory_chunks WHERE id IN ({placeholders}) AND is_deprecated = 0"
                        )
                        meta_params = list(matched_ids)
                        if session_id:
                            sql_meta += " AND session_id = ?"
                            meta_params.append(session_id)
                        meta_rows = conn.execute(sql_meta, meta_params).fetchall()
                        for row in meta_rows:
                            chunk_data[row["id"]] = row
                    for rank, (rid, _dist) in enumerate(ann_results):
                        if rid in chunk_data:
                            vec_ranks[rid] = rank + 1

                    # Also load metadata for FTS-only results not covered by ANN
                    fts_only_ids = [fid for fid in fts_ranks if fid not in chunk_data]
                    if fts_only_ids:
                        ph2 = ",".join("?" * len(fts_only_ids))
                        fts_meta_sql = (
                            f"SELECT id, content, metadata, embedding, session_id, timestamp, importance_score "
                            f"FROM memory_chunks WHERE id IN ({ph2}) AND is_deprecated = 0"
                        )
                        for row in conn.execute(fts_meta_sql, fts_only_ids).fetchall():
                            chunk_data[row["id"]] = row
                else:
                    # Rust index unavailable — fall through to Python path
                    pass
            ...
            if not vec_ranks:
                ...
                if session_id:
                    ...
                    params_vec.append(session_id)

                all_chunks = conn.execute(sql_vec, params_vec).fetchall()
