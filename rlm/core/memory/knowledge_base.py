...
def _cosine_similarity(v1: List[float], v2: List[float]) -> float:
    """Cosine similarity entre dois vetores (Rust-accelerated quando disponível)."""
...
class GlobalKnowledgeBase:
    ...
    def __init__(self, db_path: str, embedding_model: str = "text-embedding-3-small"):
        ...
        # Rust-accelerated HNSW index for summary embeddings (lazy-loaded)
    ...
    def _ensure_vector_index(self, dim: int) -> None:
        """Lazily create and warm-up the Rust HNSW index from SQLite summary embeddings."""
        if self._vector_index is not None:
            return
        if not _RUST_AVAILABLE:
            return
        self._vector_index = _ark_mem.ArkheVectorIndex(dim=dim)
        try:
            with closing(sqlite3.connect(self.db_path)) as conn:
                conn.row_factory = sqlite3.Row
                rows = conn.execute(
                    "SELECT id, embedding_summary FROM kb_documents "
                    "WHERE status = 'active' AND embedding_summary IS NOT NULL AND embedding_summary != '[]'"
                ).fetchall()
            ids = []
            embeddings = []
            for row in rows:
                emb = json.loads(row["embedding_summary"])
                if emb and len(emb) == dim:
                    ids.append(row["id"])
                    embeddings.append(emb)
            if ids:
                self._vector_index.bulk_add(ids, embeddings)
                _log.debug(f"KB Rust HNSW warm-loaded with {len(ids)} summary vectors (dim={dim})")
        except Exception as e:
            _log.warn(f"Failed to warm-load KB Rust vector index: {e}")
    ...
    def add_document(
        ...
        *,
        ...
        # Index summary embedding into Rust HNSW for fast ANN search
        if _RUST_AVAILABLE and emb_summary:
            try:
                self._ensure_vector_index(len(emb_summary))
                self._vector_index.add(doc_id, emb_summary)
            except Exception as e:
                _log.warn(f"KB Rust vector index insert failed for {doc_id}: {e}")
    ...
    def update_document(
        ...
        *,
        ...
        # Update Rust HNSW index if summary changed
        if _RUST_AVAILABLE and summary is not None and emb_summary and self._vector_index is not None:
            try:
    ...
    def search_hybrid(
        ...
        *,
        ...
        with closing(sqlite3.connect(self.db_path)) as conn:
            ...
            _used_rust_path = False
            if _RUST_AVAILABLE and query_emb:
                try:
                    self._ensure_vector_index(len(query_emb))
                except Exception:
                    pass

                if self._vector_index is not None:
                    _used_rust_path = True
                    ann_results = self._vector_index.search(query_emb, top_k=30)
                    ann_ids = [r[0] for r in ann_results]

                    # Gather all IDs we need: ANN results + FTS results
                    all_needed_ids = list(set(ann_ids) | set(fts_ranks.keys()))

                    if all_needed_ids:
                        placeholders = ",".join("?" * len(all_needed_ids))
                        sql_meta = (
                            f"SELECT * FROM kb_documents WHERE id IN ({placeholders}) AND status = ?"
                        )
                        meta_params: list = list(all_needed_ids) + [status]
                        if domain:
                            sql_meta += " AND domain = ?"
                            meta_params.append(domain)
                        for row in conn.execute(sql_meta, meta_params).fetchall():
                            doc_data[row["id"]] = row

                    # Re-score ANN results with exact weighted formula
                    for did in ann_ids:
                        if did in doc_data:
                            row = doc_data[did]
