...
try:
    import arkhe_mcts as _ark_mcts
    _RUST_MCTS = True
except ImportError:
    _ark_mcts = None  # type: ignore[assignment]
    _RUST_MCTS = False
...
def default_score_fn(repl_stdout: str, repl_stderr: str | None, code: str) -> float:
    ...
    Delegates to Rust (arkhe_mcts) when available for GIL-free performance.
    """
    if _RUST_MCTS:
        return _ark_mcts.default_score(repl_stdout, repl_stderr or "", code)
...
class ProgramArchive:
    ...
    Uses Rust-accelerated niche computation and sorting (arkhe_mcts) when available.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Rust-backed archive for niche computation + sorting
        self._rust_archive = (
            _ark_mcts.ArkheProgramArchive(max_size=self.max_size)
            if _RUST_MCTS and niche_fn is None
            else None
        )
    ...
    def update(self, branches: list[BranchResult]) -> None:
        # Always maintain the Rust archive in sync
        if self._rust_archive is not None:
            rust_branches = []
            for b in branches:
                last_stdout = ""
                if b.steps:
                    last_stdout = str(b.steps[-1].get("stdout", ""))
                rust_branches.append((
                    b.branch_id,
                    b.total_score,
                    b.final_code,
                    dict(b.aggregated_metrics),
                    b.pruned_reason,
                    b.strategy_name,
                    last_stdout,
                ))
            self._rust_archive.update(rust_branches)
        ...
        def _run_branch(branch_id: int, code_steps: list[str]) -> BranchResult:
            ...
            with SandboxREPL(
                ...
                for depth_idx, code in enumerate(code_steps[: self.max_depth]):
                    ...
                    if depth_idx == 0 and (_ark_mcts.prune_first_step(heuristic_score) if _RUST_MCTS else heuristic_score <= 0):
                        with self._lock:
                            if self.event_bus:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.branches) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    _should_terminate = (
                        _ark_mcts.should_early_terminate(result.total_score, self.max_depth)
                        if _RUST_MCTS
                        else result.total_score >= self.max_depth * 4.0
                    )
                    if _should_terminate and _early_winner is None:
...
def parse_search_replace_blocks(text: str) -> list[tuple[str, str]]:
    if _RUST_MCTS:
        return _ark_mcts.parse_search_replace_blocks(text or "")
    return [
        (search.strip("\n"), replace.strip("\n"))
...
def apply_search_replace_blocks(base_code: str, blocks: list[tuple[str, str]]) -> str:
    if _RUST_MCTS:
        return _ark_mcts.apply_search_replace_blocks(base_code, blocks)
    updated = base_code
    ...
    if _RUST_MCTS:
        branch_data = [
            (
                ...
                len(b.steps),
                b.pruned_reason,
                sorted(b.aggregated_metrics.items()),
            )
            for b in branches
        ]
        code_snippets = [b.final_code for b in branches]
        return _ark_mcts.summarize_feedback(
            branch_data, code_snippets, max_branches, code_preview_chars,
        )
