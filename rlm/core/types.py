...
EnvironmentType = Literal["local", "docker", "modal", "prime", "daytona", "sandbox"]
...
class RLMChatCompletion:
    """Record of a single RLM completion call.

    Fields
    ------
    root_model     : nome do modelo backend usado.
    prompt         : prompt original (string ou dict multimodal).
    response       : resposta textual final do agente.
    usage_summary  : totais de tokens/custo desta chamada.
    execution_time : duração total em segundos (wall-clock).
    artifacts      : variáveis REPL colhidas ao fim da execução.
                     Preenchido apenas quando ``capture_artifacts=True`` é
                     passado para ``RLM.completion()``. Contém callables e
                     valores criados pelo agente — prontos para ser reutilizados
                     como ``custom_tools`` em chamadas subsequentes.
    """
...
########################################################
########   Types for Codebase Analysis Mode    #########
########################################################

ContextMode = Literal["text", "codebase"]


@dataclass
class CodebaseContext:
    """Context representing a codebase directory for code analysis mode."""

    path: str
    languages: dict[str, int]  # {language_name: file_count}
    total_files: int
    total_lines: int
