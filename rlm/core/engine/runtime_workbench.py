...
@dataclass
class AgentContext:
    """Identity context for recursive agents — threaded into memory writes.

    Reuses existing patterns: depth from BaseEnv, branch_id from SiblingBus,
    parent_session_id from RLMMemory.session_id.
    """
    depth: int = 0
    branch_id: int | None = None
    parent_session_id: str | None = None
    role: str = "root"  # root | child_serial | child_parallel
    channel: str | None = None  # originating channel (e.g. "telegram:123", "tui:default")

    def to_metadata(self) -> dict[str, Any]:
        """Returns agent identity fields to merge into memory metadata."""
        d: dict[str, Any] = {"agent_depth": self.depth, "agent_role": self.role}
        if self.branch_id is not None:
            d["agent_branch_id"] = self.branch_id
        if self.parent_session_id is not None:
            d["agent_parent_session_id"] = self.parent_session_id
        if self.channel is not None:
            d["agent_channel"] = self.channel
        return d
