...
def extract_tool_value(entry: Any) -> Any:
    """Extract the actual value from a tool entry (strips description wrapper if present)."""
    if isinstance(entry, dict) and "tool" in entry:
        return entry["tool"]
    return entry
...
class SupportsCustomTools(Protocol):
    ...
    RESERVED NAMES (cannot be used as custom tool names):
        llm_query, llm_query_batched, rlm_query, rlm_query_batched,
        FINAL_VAR, SHOW_VARS, context, history
    """
