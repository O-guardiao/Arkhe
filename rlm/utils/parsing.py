...
def find_final_answer(text: str, environment: "BaseEnv | None" = None) -> str | None:
    ...
    if match:
        ...
        if environment is not None:
            ...
            # Guard: se FINAL_VAR retornou erro (variável não existe), não encerra o loop.
            # Retornar None permite que o RLM continue e o modelo corrija.
            if final_answer.startswith("Error:"):
                return None
            if final_answer == "":
                # stderr só no caso de exceção real no execute_code — tratar como falha
                return None
            return final_answer
        return None
