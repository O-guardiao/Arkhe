...
# ---------------------------------------------------------------------------
# Operator endpoints — usados pelo TUI workbench (live mode)
# ---------------------------------------------------------------------------


@app.post("/operator/session")
async def operator_ensure_session(request: Request):
    """Cria ou recupera sessão para o TUI operator."""
    _require_admin_api_auth(request)
    body = await request.json()
    client_id = str(body.get("client_id", "tui:default")).strip() or "tui:default"
    ...
    return {
        ...
        "state_dir": str(getattr(session, "state_dir", "")),
        "metadata": dict(getattr(session, "metadata", {}) or {}),
    }


@app.get("/operator/session/{session_id}/activity")
async def operator_session_activity(session_id: str, request: Request):
    """Retorna activity payload completo da sessão para o TUI."""
    _require_admin_api_auth(request)
    sm: SessionManager = request.app.state.session_manager
    session = sm.get_session(session_id)
    if not session:
        raise HTTPException(404, f"Sessao nao encontrada: {session_id}")

    from rlm.core.observability.operator_surface import build_activity_payload

    return build_activity_payload(sm, session)


@app.post("/operator/session/{session_id}/message")
async def operator_session_message(session_id: str, request: Request):
    """Despacha prompt do operador TUI para a sessão."""
    _require_admin_api_auth(request)
    body = await request.json()
    client_id = str(body.get("client_id", "tui:default")).strip() or "tui:default"
    ...
    payload = {"text": text, "from_user": client_id}
    loop = asyncio.get_event_loop()
    try:
        result = await loop.run_in_executor(
            None,
...
@app.post("/operator/session/{session_id}/commands")
async def operator_session_command(session_id: str, request: Request):
    """Aplica comando do operador (pause, resume, checkpoint, etc.)."""
    _require_admin_api_auth(request)
    body = await request.json()
    client_id = str(body.get("client_id", "tui:default")).strip()
    ...
    if not command_type:
        raise HTTPException(400, "command_type é obrigatório")
    ...
    from rlm.core.observability.operator_surface import (
        ...
        _apply_queued_command,
    )

    env = get_runtime_environment(session)
    if env is None:
        raise HTTPException(409, f"Sessao {session_id} sem runtime environment ativo")

    supervisor = request.app.state.supervisor
    entry = {"command_type": command_type, "payload": payload, "branch_id": branch_id}

    try:
        result = _apply_queued_command(
            sm,
            session,
            ...
            branch_id=branch_id,
            entry=entry,
            origin=f"tui:{client_id}",
        )
    except (ValueError, RuntimeError) as exc:
        raise HTTPException(400, str(exc))

    return {"status": "applied", "command_type": command_type, "result": result}
